...
export function Photo({ label, scene, src, ratio = 'aspect-[4/3]', className = '' }) {
  ...
  return (
    ...
    >
      ...
      {imgOk ? null : (
      <div className="absolute bottom-2.5 left-3 flex items-center gap-1.5 text-[12px] text-rc-teal-dark/70 font-medium bg-white/60 rounded-full px-2 py-0.5 backdrop-blur-sm">
    ...
    <header className="sticky top-0 z-30 bg-rc-ivory/90 backdrop-blur border-b border-rc-sand">
      <div className="max-w-6xl mx-auto flex items-center gap-4 px-4 md:px-6 h-[4.5rem]">
        ...
        <nav className="hidden md:flex items-center gap-5 ml-auto" aria-label="メイン">
          {nav.map(n => (
            <Link key={n.href} href={n.href} className="text-[14px] text-rc-ink-soft hover:text-rc-teal transition-colors">
              {n.label}
            </Link>
          ))}
          <button
            onClick={() => openChat()}
            className="text-[14px] font-bold text-rc-teal border border-rc-teal rounded-full px-4 py-1.5 hover:bg-rc-teal-soft transition-colors"
          >
            チャットで相談
          </button>
          <Link
            href="/recruit/entry"
            className="text-[14px] font-bold bg-rc-teal text-white rounded-full px-5 py-2 hover:bg-rc-teal-dark transition-colors shadow-sm"
          >
            応募する
          </Link>
        </nav>
        <Link
          href="/recruit/entry"
          className="md:hidden ml-auto text-[14px] font-bold bg-rc-teal text-white rounded-full px-4 py-2"
        >
          応募する
        </Link>
      </div>
...
function StickyBar() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 inset-x-0 z-30 grid grid-cols-2 gap-px bg-rc-sand border-t border-rc-sand" role="navigation" aria-label="応募・相談">
      <Link href="/recruit/entry" className="bg-rc-teal text-white text-center text-[15px] font-bold py-4 active:bg-rc-teal-dark">
        応募する
      </Link>
      <button onClick={() => openChat()} className="bg-white text-rc-teal text-center text-[15px] font-bold py-4 active:bg-rc-teal-soft">
        チャットで相談
      </button>
    </div>
  );
  ...
  return (
    <footer className="bg-rc-teal-dark text-white/90 mt-0">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-12 pb-28 md:pb-12">
        ...
        <p className="text-[12px] text-white/40 mt-8">{clinic.corporation}（採用に関する情報は本サイトに集約しています）</p>
      </div>
    </footer>
  );
}
